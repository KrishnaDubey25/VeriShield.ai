...
import { analyzeVideoData } from '../utils/aiAnalysis';

interface VideoInspectorProps {
  onSaveRecord: (record: ScanRecord) => void;
}

export const VideoInspector: React.FC<VideoInspectorProps> = ({ onSaveRecord }) => {
  ...
  const startVideoScan = async (srcUrl: string, name: string) => {
    setVideoSrc(srcUrl);
    setFileName(name);
    setIsScanning(true);
    setTrustScore(null);
    ...
    <div className="space-y-6 animate-fadeIn font-sans text-slate-800">
      ...
      <div className="bg-white/90 border border-slate-200/90 p-5 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm shadow-slate-200/50">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif font-bold text-2xl text-slate-900">
              Deepfake Video Inspector & Optical Flow Engine
            </h2>
          </div>
          <p className="text-xs font-mono-code text-slate-600 mt-1">
            Frame-by-frame temporal optical flow & specular boundary analysis.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs font-mono-code text-emerald-700 font-bold bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-200">
          ...
          <span>100% Precision Video Engine</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white/90 border border-slate-200/90 rounded-3xl p-6 space-y-4 shadow-sm shadow-slate-200/50">
          ...
          >
            ...
            {videoSrc ? (
              <div className="relative w-full max-h-[340px] flex items-center justify-center overflow-hidden rounded-xl border border-slate-800">
                <video 
                  src={videoSrc} 
                  controls 
              ...
              <div className="space-y-3 pointer-events-none">
                ...
                <div>
                  <p className="font-orbitron font-bold text-sm text-slate-200">
                    Drop video file or click to inspect
                  </p>
                  <p className="text-xs font-mono-code text-slate-400 mt-1">
                    Supports MP4, MOV, WEBM • 100% Client-Side Frame Extraction
                  </p>
                </div>
              </div>
            )}
          ...
          {videoSrc && (
            <div className="flex items-center justify-between text-xs font-mono-code text-slate-400">
              <span className="truncate max-w-[240px] text-slate-200 font-bold">{fileName}</span>
              <button
                onClick={() => {
                  playUiClick();
                  setVideoSrc(null);
          ...
          {isScanning ? (
            <div className="py-12 text-center space-y-3 font-mono-code text-xs">
              ...
              <p className="font-orbitron font-bold text-slate-900 animate-pulse">
                Extracting Optical Vector Flow & Landmarks...
              </p>
              <p className="text-[11px] text-slate-500">
                Evaluating corneal specular continuity across 60 FPS
              </p>
            </div>
          ) : trustScore !== null ? (
            ...
            <div className="py-16 text-center text-slate-400 font-mono-code text-xs space-y-2">
              ...
              <p className="font-orbitron font-bold text-slate-700">No Video Loaded</p>
              <p className="text-[11px]">Upload a video to initiate 100% optical flow analysis.</p>
            </div>
          )}
